...
class CloseBeforeRenameState(client: Actor, tag: Integer, logger: CompositeLogger, folder: Item, name: String) extends AbstractState(client, tag, logger){
  ...
  override def onOK ={
    setState(new RenameFolderState(client, tag.intValue + 1, logger, folder, name))
  }
}